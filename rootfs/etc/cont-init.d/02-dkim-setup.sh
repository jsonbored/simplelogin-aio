...
if [[ -L ${PRIVATE_KEY} ]]; then
	...
	if [[ -z ${EMAIL_DOMAIN-} ]]; then
		echo -e "${RED}[ERROR] EMAIL_DOMAIN is not set. Cannot generate DKIM keys.${NC}"
		exit 1
	fi
	...
	chown simplelogin:simplelogin "${PUBLIC_KEY}"
	...
	chown simplelogin:simplelogin "${DNS_RECORD_FILE}"
