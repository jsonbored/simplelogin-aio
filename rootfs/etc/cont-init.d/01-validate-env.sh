...
brevo)
	[[ -n ${BREVO_USERNAME-} ]] || {
		echo -e "${RED}[ERROR] BREVO_USERNAME is required when RELAY_MODE=brevo.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	[[ -n ${BREVO_PASSWORD-} ]] || {
		echo -e "${RED}[ERROR] BREVO_PASSWORD is required when RELAY_MODE=brevo.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	;;
protonmail)
	[[ -n ${PROTONMAIL_TOKEN-} ]] || {
		echo -e "${RED}[ERROR] PROTONMAIL_TOKEN is required when RELAY_MODE=protonmail.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	;;
gmail)
	[[ -n ${GMAIL_USERNAME-} ]] || {
		echo -e "${RED}[ERROR] GMAIL_USERNAME is required when RELAY_MODE=gmail.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	[[ -n ${GMAIL_APP_PASSWORD-} ]] || {
		echo -e "${RED}[ERROR] GMAIL_APP_PASSWORD is required when RELAY_MODE=gmail.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	;;
mailgun)
	[[ -n ${MAILGUN_USERNAME-} ]] || {
		echo -e "${RED}[ERROR] MAILGUN_USERNAME is required when RELAY_MODE=mailgun.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	[[ -n ${MAILGUN_PASSWORD-} ]] || {
		echo -e "${RED}[ERROR] MAILGUN_PASSWORD is required when RELAY_MODE=mailgun.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	;;
custom)
	[[ -n ${CUSTOM_RELAYHOST-} ]] || {
		echo -e "${RED}[ERROR] CUSTOM_RELAYHOST is required when RELAY_MODE=custom.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	[[ -n ${CUSTOM_USERNAME-} ]] || {
		echo -e "${RED}[ERROR] CUSTOM_USERNAME is required when RELAY_MODE=custom.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	[[ -n ${CUSTOM_PASSWORD-} ]] || {
		echo -e "${RED}[ERROR] CUSTOM_PASSWORD is required when RELAY_MODE=custom.${NC}"
		ERROR_COUNT=$((ERROR_COUNT + 1))
	}
	;;
*)
