...
wait_for_log() {
	...
	while ((SECONDS < deadline)); do
		if docker logs "${CONTAINER_NAME}" 2>&1 | grep -F "${expected_log}" >/dev/null; then
			return 0
		fi
		sleep 2
	done
	echo "Case '${CASE_NAME}' did not emit expected log '${expected_log}' in time." >&2
	docker logs "${CONTAINER_NAME}" || true
	return 1
}
...
run_failure_case() {
	...
	echo "==> ${case_name}"
	start_case "${case_name}" "$@"
	wait_for_log "${expected_log}"
	if docker exec "${CONTAINER_NAME}" sh -lc "curl -fsS http://127.0.0.1:7777/health >/dev/null" >/dev/null 2>&1; then
		echo "Case '${CASE_NAME}' unexpectedly became healthy." >&2
		docker logs "${CONTAINER_NAME}" || true
		return 1
	fi
	cleanup_case
...
run_case "legacy-admin-fido-no-leading-pipe" "" \
	-e ADMIN_FIDO_REQUIRED="none|any|hardware"

run_case "blank-admin-fido" "" \
	-e ADMIN_FIDO_REQUIRED=""
...
run_case "oidc-keygen" "python -c \"from pathlib import Path; paths={'/appdata/sl/jwtRS256.key':'0o600','/appdata/sl/jwtRS256.key.pub':'0o644'}; [((lambda p,e: (p.is_file() and oct(p.stat().st_mode & 0o777)==e) or (_ for _ in ()).throw(SystemExit(f'{p}:{oct(p.stat().st_mode & 0o777) if p.exists() else \\\"missing\\\"}!={e}')))(Path(path), expected)) for path, expected in paths.items()]\"" \
	-e ENABLE_OIDC_SERVER="1"

run_case "pgp-keygen" "python -c \"from pathlib import Path; paths={'/pgp/server_private_key.asc':'0o600','/pgp/server_public_key.asc':'0o644'}; [((lambda p,e: (p.is_file() and oct(p.stat().st_mode & 0o777)==e) or (_ for _ in ()).throw(SystemExit(f'{p}:{oct(p.stat().st_mode & 0o777) if p.exists() else \\\"missing\\\"}!={e}')))(Path(path), expected)) for path, expected in paths.items()]\"" \
...
run_failure_case "invalid-admin-fido" "ADMIN_FIDO_REQUIRED is not a valid value" \
	-e ADMIN_FIDO_REQUIRED="definitely-not-valid"

run_failure_case "invalid-relay-mode" "RELAY_MODE=bogus is not supported" \
	-e RELAY_MODE="bogus"

run_failure_case "missing-url" "[ERROR] URL is not set." \
	-e URL=""
